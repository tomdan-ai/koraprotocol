...
export default function TopMovers({ markets = [], onSelect }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-100 bg-white shadow-sm dark:bg-[#071018] dark:border-zinc-800">
        {top.map((m, idx) => (
          <button
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <div className={`text-sm font-semibold ${m.change && m.change.toString().startsWith('-') ? 'text-rose-500' : 'text-emerald-600'}`}>
                {m.change ?? '+0.0%'}
              </div>
            </div>
          </button>
